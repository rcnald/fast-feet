...
describe("Upload and create attachment", () => {
  ...
  it("should be able to upload and create a attachment", async () => {
    const [error, result] = await sut.execute({ fileName: "attachment-name", fileType: "image/png", body: Buffer.from("") })

    expect(error).toEqual(undefined)
    expect(result?.attachment).toEqual(expect.objectContaining({ title: "attachment-name" }))
  })

  it("should not be able to upload and create a attachment with wrong file type", async () => {
    const [error] = await sut.execute({ fileName: "attachment-name", fileType: "audio/mp3", body: Buffer.from("") })
